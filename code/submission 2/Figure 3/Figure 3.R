###############################################################################
# Interrupted Time Series (ITS) Segmented Model for Monthly Waitlist Size
# Dataset: monthly_waitlist_size.csv
# Columns (required):
#   - month                (e.g., "1/1/15")
...
#   - EXACT totals for Jun 2023 – May 2024
#
# Update:
...
csv_file          <- file.path(here(), "code", "submission 2", 
                                       "Figure 3", "data", "monthly_waitlist_size.csv")
...
df <- df_raw %>%
  ...
  mutate(
    ...
    knot62     = pmax(0, time - knots[1]),
    knot65     = pmax(0, time - knots[2])
  )

model <- lm(WaitlistPM ~ time + knot62 + knot65 + time_after, data = df)
...
label_x_offset_days <- 105     # positive = right, negative = left
...
p <- ggplot(df, aes(Date)) +
  ...
  labs(title = "Monthly Active Waitlist Census: Kidney",
...
if(FALSE){
  ggsave(file.path(here(), "code", "submission 2", "Figure 3", "Figure 3.pdf"), 
         p, 
         device = "pdf", 
         height = 8.5, 
         width = 8.5, 
...
segments <- list(
  "Pre-knot62" = c("time"),
  "Knot62-65"  = c("time","knot62"),
  "Knot65-Int" = c("time","knot62","knot65"),
  "Post-Int"   = c("time","knot62","knot65","time_after")
)
...
final_vars <- c("time","knot62","knot65","time_after")
...
###############################################################################
# --- EXACT TOTALS (Jun 2023–May 2024): modeled vs expected with CIs ---
###############################################################################

post_start <- as.Date("2023-06-01")
post_end   <- as.Date("2024-05-01")
