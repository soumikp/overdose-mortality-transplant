...
file <- file.path(here(), "code", "submission 2", 
                  "SupplementaryFigure 4", "data", "Liver_tx_waitlist_denominator_OD_nonOD_data.csv")
...
for (panel_name in levels(df_plotB$Panel)) {
  
  panel_data <- df_plotB %>% filter(Panel == panel_name)
...
smooth_lines_dfB <- bind_rows(smooth_lines_list)

smooth_lines_dfB$Panel <- factor(
  smooth_lines_dfB$Panel,
  levels = c("Drug Overdose", "Non-drug Overdose", "Overall")
)
...
p <- ggplot(df_plotB, aes(x = Date)) +
  geom_point(aes(y = Observed), color = "black", size = 1.5) +
  geom_line(
    data = smooth_lines_dfB,
