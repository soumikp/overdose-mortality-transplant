...
file <- file.path(here(), "code", "submission 2", 
                  "SupplementaryFigure 4", "data", "Kidney_tx_joinpoint_waitlist_denomainator_OD_nonOD_data.csv")
...
# --------------------------
# 2. Filter to Donor_MoD 1, 2, 7 & create dates
# --------------------------
...
for (panel_name in levels(df_plotA$Panel)) {
  
  panel_data <- df_plotA %>% filter(Panel == panel_name)
...
smooth_lines_dfA <- bind_rows(smooth_lines_list)

smooth_lines_dfA$Panel <- factor(
  smooth_lines_dfA$Panel,
  levels = c("Drug Overdose", "Non-drug Overdose", "Overall")
)
...
p <- ggplot(df_plotA, aes(x = Date)) +
  geom_point(aes(y = Observed), color = "black", size = 1.5) +
  geom_line(
    data = smooth_lines_dfA,
