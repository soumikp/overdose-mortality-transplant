################################################################################
# TITLE: Monthly Change in Overdose Deaths vs. ODD Kidney & Liver Transplants
################################################################################
rm(list = ls())
...
df <- read_csv(
  file.path(here(), "code", "submission 2", 
            "Figure 5", "Mortality_Vs_Transplant.csv"),
  col_types = cols(
...
#   data      = df_kidney,
#   y_var     = "dKidney",
#   title_main = "Monthly Change in Overdose Deaths vs. ODD Kidney Transplants",
...
liver_results <- run_hac_model(
  data      = df_liver,
  y_var     = "dLiver",
  title_main = stringr::str_wrap("Monthly Change in Overdose Deaths vs. Overdose Decedent Donor Liver Transplants", width = 60),
  label_y   = "Change in 12-month-ending liver transplants (per million)"
)

p <- liver_results$plot

if(FALSE){
  ggsave(file.path(here(), "code", "submission 2", "Figure 5", "Figure 5.pdf"), 
         p, 
         device = "pdf", 
         height = 8.5, 
         width = 8.5, 
