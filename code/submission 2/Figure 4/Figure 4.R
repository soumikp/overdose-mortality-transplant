...
file <- file.path(here(), "code", "submission 2", "Figure 4", "data", "DDLT_joinpoint_data.csv")

df <- read_tsv(file, show_col_types = FALSE)

names(df) <- gsub("\ufeff", "", names(df))
names(df) <- gsub("'d4'aa'f8", "", names(df))
...
df_plot |> filter(Donor_MoD == 2) |> filter(Month == 18)
df_plot |> filter(Donor_MoD == 1) |> filter(Model == max(Model))
df_plot |> filter(Donor_MoD == 1) |> filter(Month == max(Month))
